...
Given the recent messages, extract the following information about the requested get entry functions:
- module name
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ...
        console.log("Validating GET_ENTRY_FUNCTIONS from user: ", message.userId, workspace);
    ...
    description: "Get Move entry functions",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_ENTRY_FUNCTIONS action.");
            if (callback) {
                callback({
                    text: "Unable to process get entry functions request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_entry_functions(module);
        elizaLogger.log("Starting GET_ENTRY_FUNCTIONS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following entry functions \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get entry functions:", error);
            if (callback) {
                callback({
                    text: `Error get entry functions: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get entry functions for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you entry functions for this move module",
                    action: "GET_ENTRY_FUNCTIONS",
                },
            }
        ]
    ] as ActionExample[][],
