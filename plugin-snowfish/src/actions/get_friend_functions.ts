...
Given the recent messages, extract the following information about the requested get friend functions:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "GET_FRIEND_FUNCTIONS",
    similes: [
        "QUERY_FRIEND_FUNCTIONS"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating GET_FRIEND_FUNCTIONS from user: ", message.userId, workspace);
    ...
    description: "Get Move friend functions",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_FRIEND_FUNCTIONS action.");
            if (callback) {
                callback({
                    text: "Unable to process get friend functions request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_friend_functions(module);
        elizaLogger.log("Starting GET_FRIEND_FUNCTIONS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following friend functions \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get friend functions:", error);
            if (callback) {
                callback({
                    text: `Error get friend functions: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get friend functions for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you friend functions for this move module",
                    action: "GET_FRIEND_FUNCTIONS",
                },
            }
        ]
    ] as ActionExample[][],
