...
Given the recent messages, extract the following information about the requested get private functions:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "GET_PRIVATE_FUNCTIONS",
    similes: [
        "GET_PRIVATE_FUNCTIONS"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating GET_PRIVATE_FUNCTIONS from user: ", message.userId, workspace);
    ...
    description: "Get Move private functions",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_PRIVATE_FUNCTIONS action.");
            if (callback) {
                callback({
                    text: "Unable to process get private functions request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_private_functions(module);
        elizaLogger.log("Starting GET_PRIVATE_FUNCTIONS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following private functions \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get private functions:", error);
            if (callback) {
                callback({
                    text: `Error get private functions: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get private functions for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you private functions for this move module",
                    action: "GET_PRIVATE_FUNCTIONS",
                },
            }
        ]
    ] as ActionExample[][],
