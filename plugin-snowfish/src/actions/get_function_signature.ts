...
function isContent(content: Content): content is MessageContent {
    return typeof content.module === "string" && typeof content.function_name === "string";
}
...
Given the recent messages, extract the following information about the requested get function signature:
- module name
- function name
...
export default {
    ...
    similes: [
        "QUERY_FUNCTION_SIGNATURE"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating GET_FUNCTION_SIGNATURE from user: ", message.userId, workspace);
    ...
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_FUNCTION_SIGNATURE action.");
            if (callback) {
                callback({
                    text: "Unable to process get move function signature request. Invalid content provided.",
        ...
        let function_name = messageContent.function_name;
        ...
        let f_results = move_code_helper.disassemble_function(module, function_name);
        elizaLogger.log("Starting GET_FUNCTION_SIGNATURE handler...");

        try {
            let text = `The ${module} Move Module has the following move function signature \n ${f_results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get function signature:", error);
            if (callback) {
                callback({
                    text: `Error get function signature: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get the function signature for the move function in the move module"
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you move function signature for this move module",
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get the function signature for the set_foo5 function in the wasm_test module"
                },
            },
            {
                user: "{{user2}}",
