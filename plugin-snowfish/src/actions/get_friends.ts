...
Given the recent messages, extract the following information about the requested get module friends:
- module name
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ...
        console.log("Validating GET_FRIENDS from user: ", message.userId, workspace);
    ...
    description: "Get Move friends Modules",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_FRIENDS action.");
            if (callback) {
                callback({
                    text: "Unable to process get friends request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_friends(module);
        elizaLogger.log("Starting GET_FRIENDS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following friends modules \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            if (callback) {
                callback({
                    text: `Error get friends: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get friends modules declared in this move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you friends modules for this move module",
                    action: "GET_FRIENDS",
                },
            }
        ]
