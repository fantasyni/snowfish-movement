...
Given the recent messages, extract the following information about the requested get module unused private functions:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "UNUSED_PRIVATE_FUNCTIONS",
    similes: [
        "UNUSED_PRIVATE_FUNCTIONS"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating UNUSED_PRIVATE_FUNCTIONS from user: ", message.userId, workspace);
    ...
    description: "Get Move Module Unused Private Functions",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for UNUSED_PRIVATE_FUNCTIONS action.");
            if (callback) {
                callback({
                    text: "Unable to process get module unused private functions request. Invalid content provided.",
        ...
        let f_results = move_code_helper.unused_private_functions(module);
        elizaLogger.log("Starting UNUSED_PRIVATE_FUNCTIONS handler...");

        try {
            let results = JSON.parse(f_results).join("\n");

            let text = `The ${module} Move Module has the following unused private functions \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get module unused private functions:", error);
            if (callback) {
                callback({
                    text: `Error get module unused private functions: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get unused private functions for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you unused private functions for this move module",
                    action: "UNUSED_PRIVATE_FUNCTIONS",
                },
            }
        ]
    ] as ActionExample[][],
