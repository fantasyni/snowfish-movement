...
Given the recent messages, extract the following information about the requested get public functions:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "GET_PUBLIC_FUNCTIONS",
    similes: [
        "GET_PUBLIC_FUNCTIONS"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating GET_PUBLIC_FUNCTIONS from user: ", message.userId, workspace);
    ...
    description: "Get Move public functions",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_PUBLIC_FUNCTIONS action.");
            if (callback) {
                callback({
                    text: "Unable to process get public functions request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_public_functions(module);
        elizaLogger.log("Starting GET_PUBLIC_FUNCTIONS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following public functions \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get public functions:", error);
            if (callback) {
                callback({
                    text: `Error get public functions: ${error.message}`,
