...
Given the recent messages, extract the following information about the requested get module objects structs:
- module name
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ...
        console.log("Validating GET_OBJECTS from user: ", message.userId, workspace);
    ...
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_OBJECTS action.");
            if (callback) {
                callback({
                    text: "Unable to process get objects request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_objects(module);
        elizaLogger.log("Starting GET_OBJECTS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following objects structs \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get objects:", error);
            if (callback) {
                callback({
                    text: `Error get objects: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get objects structs for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you objects structs for this move module",
                    action: "GET_OBJECTS",
                },
            }
        ]
