...
Given the recent messages, extract the following information about the requested get phantom structs:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "GET_PHANTOM_STRUCTS",
    similes: [
        "GET_PHANTOM_STRUCTS"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating GET_PHANTOM_STRUCTS from user: ", message.userId, workspace);
    ...
    description: "Get Move phantom structs",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_PHANTOM_STRUCTS action.");
            if (callback) {
                callback({
                    text: "Unable to process get phantom structs request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_phantom_structs(module);
        elizaLogger.log("Starting GET_PHANTOM_STRUCTS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following phantom structs \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get phantom structs:", error);
            if (callback) {
                callback({
                    text: `Error get phantom structs: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get phantom structs for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you phantom structs for this move module",
                    action: "GET_PHANTOM_STRUCTS",
                },
            }
        ]
    ] as ActionExample[][],
