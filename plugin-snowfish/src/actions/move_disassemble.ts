...
Given the recent messages, extract the following information about the requested get module disassembled move code:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "MOVE_DISASSEMBLE",
    similes: [
        "MOVE_DISASSEMBLE"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating MOVE_DISASSEMBLE from user: ", message.userId, workspace);
    ...
    description: "Get Move MOVE_DISASSEMBLE Code",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for MOVE_DISASSEMBLE action.");
            if (callback) {
                callback({
                    text: "Unable to process get move disassembled code request. Invalid content provided.",
        ...
        let f_results = move_code_helper.move_disassemble(module);
        elizaLogger.log("Starting MOVE_DISASSEMBLE handler...");

        try {
            let text = `The ${module} Move Module has the following move disassembled code \n ${f_results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get move disassembled code:", error);
            if (callback) {
                callback({
                    text: `Error get move disassembled code: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get move disassembled code for move module",
                },
            },
            {
                user: "{{user2}}",
