...
Given the recent messages, extract the following information about the requested get module unused constant:
- module name
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ...
        console.log("Validating UNUSED_CONSTANT from user: ", message.userId, workspace);
    ...
    description: "Get Move Module Unused Constant",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for UNUSED_CONSTANT action.");
            if (callback) {
                callback({
                    text: "Unable to process get module unused constant request. Invalid content provided.",
        ...
        let f_results = move_code_helper.unused_constant(module);
        elizaLogger.log("Starting UNUSED_CONSTANT handler...");
        ...
        try {
            ...
            for(var i = 0;i<r_results.length;i+=2) {
                ...
                let constant_name = r_results[i+1];
                results += (`function: ${func_name} \n      ${JSON.parse(constant_name).join(", ")}\n`)
            }

            let text = `The ${module} Move Module has the following unused constant \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get module unused constant:", error);
            if (callback) {
                callback({
                    text: `Error get module unused constant: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get unused constant for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you unused constant for this move module",
                    action: "UNUSED_CONSTANT",
                },
            }
        ]
    ] as ActionExample[][],
