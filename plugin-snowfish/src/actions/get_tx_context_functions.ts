...
Given the recent messages, extract the following information about the requested get tx_context functions:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "GET_TX_CONTEXT_FUNCTIONS",
    similes: [
        "GET_TX_CONTEXT_FUNCTIONS"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating GET_TX_CONTEXT_FUNCTIONS from user: ", message.userId, workspace);
    ...
    description: "Get Move tx_context functions",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_TX_CONTEXT_FUNCTIONS action.");
            if (callback) {
                callback({
                    text: "Unable to process get tx_context functions request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_tx_context_functions(module);
        elizaLogger.log("Starting GET_TX_CONTEXT_FUNCTIONS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following tx_context functions \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get tx_context functions:", error);
            if (callback) {
                callback({
                    text: `Error get tx_context functions: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get tx_context functions for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you tx_context functions for this move module",
                    action: "GET_TX_CONTEXT_FUNCTIONS",
                },
            }
        ]
    ] as ActionExample[][],
