...
Given the recent messages, extract the following information about the requested get module unchecked return functions:
- module name
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_WORKSPACE");
        console.log("Validating UNCHECKED_RETURN from user: ", message.userId, workspace);
    ...
    description: "Get Move Module Unchecked Return Functions",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for UNCHECKED_RETURN action.");
            if (callback) {
                callback({
                    text: "Unable to process get module unchecked return functions request. Invalid content provided.",
        ...
        let f_results = move_code_helper.unchecked_return(module);
        elizaLogger.log("Starting UNCHECKED_RETURN handler...");
        ...
        try {
            ...
            for(var i = 0;i<r_results.length;i+=2) {
                ...
                let call_name = r_results[i+1];
                results += (`function: ${func_name} \n      ${JSON.parse(call_name).join(", ")}\n`)
            }

            let text = `The ${module} Move Module has the following unchecked return functions \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get module unchecked return functions:", error);
            if (callback) {
                callback({
                    text: `Error get module unchecked return functions: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get unchecked return functions for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you unchecked return functions for this move module",
                    action: "UNCHECKED_RETURN",
                },
            }
        ]
    ] as ActionExample[][],
