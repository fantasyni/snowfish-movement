...
Given the recent messages, extract the following information about the requested get frozen objects:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "GET_FROZEN_OBJECTS",
    similes: [
        "QUERY_FROZEN_OBJECTS"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating GET_FROZEN_OBJECTS from user: ", message.userId, workspace);
    ...
    description: "Get Move Frozen Objects",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_FROZEN_OBJECTS action.");
            if (callback) {
                callback({
                    text: "Unable to process get frozen objects request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_frozen_objects(module, false);
        elizaLogger.log("Starting GET_FROZEN_OBJECTS handler...");
        ...
        try {
            ...
            let text = `The ${module} Move Module has the following frozen objects \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get frozen objects:", error);
            if (callback) {
                callback({
                    text: `Error get frozen objects: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get frozen objects for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you frozen objects for this move module",
                    action: "GET_FROZEN_OBJECTS",
                },
            }
        ]
    ] as ActionExample[][],
