...
Given the recent messages, extract the following information about the requested get transferred objects:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "GET_TRANSFERRED_OBJECTS",
    similes: [
        "QUERY_TRANSFERRED_OBJECTS"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating GET_TRANSFERRED_OBJECTS from user: ", message.userId, workspace);
    ...
    description: "Get Move Transferred Objects",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_TRANSFERRED_OBJECTS action.");
            if (callback) {
                callback({
                    text: "Unable to process get transferred objects request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_transferred_objects(module, false);
        elizaLogger.log("Starting GET_TRANSFERRED_OBJECTS handler...");
        ...
        try {
            ...
            let text = `The ${module} Move Module has the following transferred objects \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get transferred objects:", error);
            if (callback) {
                callback({
                    text: `Error get transferred objects: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get transferred objects for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you transferred objects for this move module",
                    action: "GET_TRANSFERRED_OBJECTS",
                },
            }
        ]
    ] as ActionExample[][],
