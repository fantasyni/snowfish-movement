...
Given the recent messages, extract the following information about the requested get module functions:
- module name
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ...
        console.log("Validating GET_FUNCTIONS from user: ", message.userId, workspace);
    ...
    description: "Get Move Functions",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_FUNCTIONS action.");
            if (callback) {
                callback({
                    text: "Unable to process get function request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_functions(module);
        elizaLogger.log("Starting GET_FUNCTIONS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following functions \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get functions:", error);
            if (callback) {
                callback({
                    text: `Error get functions: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get functions declared in move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you functions in this move module",
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "what about move module's functions, show me in this module",
                },
            },
            {
                user: "{{user2}}",
