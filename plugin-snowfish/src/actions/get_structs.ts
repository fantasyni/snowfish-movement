...
Given the recent messages, extract the following information about the requested get module struct:
- module name
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ...
        console.log("Validating GET_STRUCTS from user: ", message.userId, workspace);
    ...
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_STRUCTS action.");
            if (callback) {
                callback({
                    text: "Unable to process get structs request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_structs(module);
        elizaLogger.log("Starting GET_STRUCTS handler...");

        let results = "";
        try {
            results = JSON.parse(f_results).join("\n");
            let text = `The ${module} Move Module has the following structs \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get structs:", error);
            if (callback) {
                callback({
                    text: `Error get structs: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get structs declared in move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you structs in this move module",
                    action: "GET_STRUCTS",
                },
            }
        ],
        [
            {
                user: "{{user1}}",
                content: {
                    text: "what about move module's structs, show me in this module",
                },
            },
            {
                user: "{{user2}}",
