...
Given the recent messages, extract the following information about the requested get shared objects:
- module name

Respond with a JSON markdown block containing only the extracted values.`;

export default {
    name: "GET_SHARED_OBJECTS",
    similes: [
        "QUERY_SHARED_OBJECTS"
    ],
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        const workspace = runtime.getSetting("SUI_CONTRACTS");
        console.log("Validating GET_SHARED_OBJECTS from user: ", message.userId, workspace);
    ...
    description: "Get Move Shared Objects",
    handler: async (
        ...
        if (!isContent(messageContent)) {
            console.error("Invalid content for GET_SHARED_OBJECTS action.");
            if (callback) {
                callback({
                    text: "Unable to process get shared objects request. Invalid content provided.",
        ...
        let f_results = move_code_helper.get_shared_objects(module, false);
        elizaLogger.log("Starting GET_SHARED_OBJECTS handler...");
        ...
        try {
            ...
            let text = `The ${module} Move Module has the following shared objects \n ${results}`;
            if (callback) {
                callback({
                    text: text,
                    content: {  },
            ...
            console.error("Error get shared objects:", error);
            if (callback) {
                callback({
                    text: `Error get shared objects: ${error.message}`,
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "get shared objects for move module",
                },
            },
            {
                user: "{{user2}}",
                content: {
                    text: "Ok, I will show you shared objects for this move module",
                    action: "GET_SHARED_OBJECTS",
                },
            }
        ]
    ] as ActionExample[][],
